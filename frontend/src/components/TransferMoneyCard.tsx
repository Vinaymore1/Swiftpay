...
const TransferMoneyCard: FC<TransferMoneyCardProps> = ({ recepient }) => {
  ...
  // Sync `userid` with `recepient` whenever `recepient` is updated
  useEffect(() => {
    if (recepient) {
  ...
  const transferMoney = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const res = await axiosInstance.post("/account/transfer", {
        to: values.userid, // Use `userid` directly from form values
        amount,
      });
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      value={field.value || ""}
                      onChange={(e) => field.onChange(e.target.value)} // Allow manual entry
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
